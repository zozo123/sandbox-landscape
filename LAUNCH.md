...
The framing here is that teams are not buying one sandbox. They should choose the execution contract first: permissions, isolation, reset semantics, state, scale, credential boundaries, evidence capture, and stop conditions. The vendor/runtime choice comes after that.
...
The UI uses a compact terminal-directory style, a deterministic execution-atlas canvas, and links to officialskills.sh as an adjacent agent-skills registry; this repo is about sandbox execution boundaries.
...
- Website includes a canvas execution atlas with native controls and text summaries.
